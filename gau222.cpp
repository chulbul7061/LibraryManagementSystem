        ...
        while (outf.read(reinterpret_cast<char *>(&st), sizeof(student)))
        {
            if (st.admno == sn)
            {
                ...
                if (flag == 1)
                {
                    ...
                    outf.write(reinterpret_cast<char *>(&st), sizeof(student));
                    cout << "\n\t\tBook Issued";
                    cout << "\n\t\tNote: Write the current date in the backside of the book";
                    cout << "\n\t\tShould be submitted within 15 days to avoid fine";
                    cout << "\n\t\tThe fine is Rs. 1 for each day after the 15-day period\n";
                    break;
                }
                else if (flag == 2)
                {
        ...
        while (outf.read(reinterpret_cast<char *>(&st), sizeof(student)))
        {
            if (st.admno == sn)
            {
                ...
                if (flag)
                {
                    ...
                    outf.write(reinterpret_cast<char *>(&st), sizeof(student));
                    int days;
                    cout << "\n\t\tBook deposited in the number of days:";
                    cin >> days;
                    if (days > 15)
                    {
                        int fine = (days - 15) * 1;
